...
- **Sprout type rotation.** v1 cycles deterministically by sprout index: 0 → tree, 1 → flower, 2 → fruit (attaching to nearest `BUSH_PLACEMENTS` bush, NOT a tree — `Fruits.js` is bush-bound today), 3 → tree, … Rationale: predictable, no taxonomy needed, easy to override in v2 when claim dimension is available. **This rotation is the highest-risk product decision in the plan — see Risks table.** Open question carried into review: whether to drop the rotation and ship v1 single-species ("sprout → tree only") to avoid Tamagotchi-shaped feedback before v2 lands claim binding.
...
### Open Product Decisions (Surfaced by Review)

- **Species rotation vs single-species v1.** Doc review flagged that the deterministic rotation (tree/flower/fruit by sprout index) ships a Tamagotchi-shaped feedback loop in conflict with the brainstorm's "Outside this product's identity" stance. v1 alternatives: (a) keep rotation as planned, (b) ship single-species v1 (sprout → tree only) and introduce species variety in v2 when claim dimension is available. **Pending decision.**
- **Reduced-motion celebratory beat.** The 200ms cross-fade is the only beat for reduced-motion users (no chime, no camera fly-in, no particle burst). Risks reading as a rendering glitch. Should v1 add a non-motion celebratory beat — for example, a brief screen-flash, a high-contrast color pulse on the new object, or a subtle haptic on touch devices — to land the moment? **Pending decision.**
- **Toast copy voice.** "+1 toward {species} ({n}/{threshold})" reads as gamified. Alternative copy that drops the points-style register: e.g., `"Heard. Something is growing."` for `grew`, `"This one's ready to plant."` for `markedReady`. Voice decision belongs upstream of implementation. **Pending decision.**
- **Alternative VIPS path (post-session payload).** The plan rejected the synchronous-Connector-in-v1 alternative. A separate path — async post-session payload, no SSE, no Mirror UI on home — was not weighed. This could ship the brainstorm's claim-bound product without staging through a generic-progression v1. **Pending decision (and possibly re-brainstorm).**
...
- Schema: `Sprout = { id, createdAt, species: 'tree'|'flower'|'fruit', placementSeed: number, threshold: number, count: number, readyToBloom: boolean, bloomedAt: string|null, captureRefs: string[] }`. `captureRefs` holds capture IDs for traceability and v2 claim binding (forward-compatible: v2 derives claim binding from captureRefs[i] → mirror entry → Connector decision → claim).
- Threshold + rotation constants: inline at the top of `Sprouts.js` as `THRESHOLDS = Object.freeze({ tree: 3, flower: 3, fruit: 2 })` and `ROTATION = Object.freeze(['tree', 'flower', 'fruit'])` — single consumer, no separate config file needed for v1.
...
- Modify: `src/engine/student-space/Game/View/Flowers.js` (same)
- Modify: `src/engine/student-space/Game/View/Fruits.js` (same — fruit attaches to nearest BUSH_PLACEMENTS bush; existing fruit-on-bush model)
- Modify: `src/engine/student-space/Game/View/ObjectPeek.js` (add a `sprout` entry to `KIND_CONFIG` — copy: eyebrow="growing", title from species name, meaning = capture count, primary CTA = "Done" returning camera; or a slim peek-only variant that bypasses the Kira companion phase. Pick during implementation.)
...
**Approach:**
- **InstancedMesh runtime-spawn strategy.** Tree.js / Flowers.js / Fruits.js build a single `THREE.InstancedMesh` per species, sized to a fixed `PLACEMENTS` (or `BUSH_PLACEMENTS`) array at boot. v1 chooses **pre-allocate spare slots**: extend each module's PLACEMENTS array with N reserved slots (default N=12) marked `hidden=true` at boot. The new method `revealAt(slotIndex, worldX, worldZ)` writes the matrix for the reserved slot, sets `instanceMatrix.needsUpdate`, and calls the existing `growIn(index)` tween. Rationale: zero GPU resource churn per bloom, deterministic memory ceiling, works with existing `growIn`. Documented trade: caps total user-bloomed objects per species at N=12; once exhausted the bloom logs and the tray rejects further blooms of that species (rare path; v2 raises the ceiling).
...
| Fruit rotation collides with bush model (Fruits.js attaches to BUSH_PLACEMENTS, not trees) | Rotation 0/1/2 = tree/flower/fruit; fruit attaches to the nearest BUSH_PLACEMENTS slot, matching the engine's existing fruit-on-bush semantics. v2 can re-shuffle when claim dimensions decide species. |
| Tamagotchi-shaped feedback ships in v1 because species rotation is purely activity-driven | **Open product decision — see Open Questions below.** Mitigation if accepted: ship v1 single-species (tree only) and defer species variety to v2. Mitigation if rotation stays: adjust toast copy to drop the "+1 toward {species}" register. |

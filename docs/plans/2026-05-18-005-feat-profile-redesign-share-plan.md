...
- `src/engine/student-space/Game/State/Profile.js` — localStorage-backed singleton. `countByClaim(facetId)` returns per-claim quote counts. Identity (`Alice`, `Sec 3B`, avatar) lives here.
...
- **Demo and dev-bypass auth-kinds cannot create real share tokens.** They get a "Sign in to share" tooltip. Avoids leaking the seeded `Alice` persona to the public web. Locks the share table to real student rows only.

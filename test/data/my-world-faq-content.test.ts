...
describe('My World FAQ content registry', () => {
  ...
  it('uses stable unique IDs, slugs, aliases, and authored order', () => {
    ...
    for (const question of FAQ_QUESTIONS) {
      ...
      expect(words(question.shortAnswer).length, question.id).toBeGreaterThanOrEqual(40)
      expect(words(question.shortAnswer).length, question.id).toBeLessThanOrEqual(80)

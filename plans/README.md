...
| 030 | **Engine (product app)**: island close-up quality — retina DPR fix (strong dpr-2 machines boot tier `high`, not `medium`/1.5×) + terrain `SEGMENTS` 256→512 (maintainer "pixelated and blocky" screenshot; plan 028's sampler fix was already ported — these are the two remaining causes) | P1 | S | none (engine-side; editor untouched) | TODO |

...
| 031 | **Engine + editor data**: curved coastline — one-time resample of the saved island 64×64→128×128 (threshold of a strongly-blurred field bakes curves into the data; runtime smoothing can't fix multi-cell staircase steps because BLUR_MIX keeps 60% raw by design) + BLADES_PER_CELL 64→16 density compensation + `pnpm sync:island` regen | P1 | M | **030** (stacks on `advisor/030-island-closeup-quality`) | IN PROGRESS (main commit `7b65a381` done after 1 revision — rev 1 authorized 4 resolution-literal test updates; rev 2 de-flake of the ~1M-expect() per-vertex geometry tests interrupted by a 529 API error, resuming) |
| 032 | **Engine + editor**: curved contours at runtime — retune tier-field sampler BLUR_PASSES 2→4, BLUR_MIX 0.4→0.85 (reviewer's offline contour study: steps stay visually hard until raw weight < ~0.2; invariant REDEFINED in world-space terms — old 64-cell minimum feature ≈ 3×3 128-cells stays a raised bump) | P1 | M | **031** | TODO (dispatching) |

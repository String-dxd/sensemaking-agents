...
- **Base UI for behavior, hand-rolled shadcn-style visuals.** `@base-ui/react` for dialogs, drawers, radio groups, focus traps. Visual primitives in `src/components/ui/*` are local. Do **not** install the `shadcn/ui` package.
- **Tenancy via `withStudent`.** Every student-owned DB read/write goes through the envelope (`src/db/*`, server handlers). Bypassing it is a tenancy bug. The only system-scoped exception is the public My World FAQ: `src/server/my-world-faq-repository.server.ts` may use the narrow `getMyWorldFaqSystemDatabase()` accessor because its three tables contain no student data. Route code must use that repository and must never call the accessor directly.

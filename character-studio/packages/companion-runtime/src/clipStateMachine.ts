...
// PORT of the studio's `src/core/motion/clipStateMachine.ts` (plan 007) with
// the three namespace INJECTED. Values that were static three imports
// (AnimationUtils, LoopOnce, LoopRepeat, AdditiveAnimationBlendMode) now come
// off the injected `THREE`; the mixer + clips come from the host's parsed
// GLTF. The arbitration/crossfade/gesture logic is otherwise identical.
import type {
  AnimationActionLike as AnimationAction,
  AnimationClipLike as AnimationClip,
  AnimationMixerLike as AnimationMixer,
  ThreeNamespace,
} from './three-types'

// Ported from island-editor/src/models/buildObjectModel.ts — behavior kept in
// sync via shared test vectors (see State/islandSpecCore/terrainGrid.ts).
//
// Stylized PROCEDURAL models built from three.js primitives — now only the
// bush (`tree` and `rock` ship as authored GLB assets through assetLoader).
// Art direction matches across lanes: matte masses with baked shading,
// TOON-LIT — baked vertex colors multiply the shared toon ramp, so the whole
// scene shades as one system under the sun + hemisphere lights.
// Deterministic given a seed. No Math.random / Date.

import * as THREE from 'three'
import { hashString, mulberry32 } from '../State/islandSpecCore/rand.ts'
import type { ObjectKind } from '../State/islandSpecCore/terrainGrid.ts'
import { objectGradientMap } from './Materials/toonMaterial.ts'
import { modelTexture } from './modelTextures.ts'
 ...
 *  normal by a small amount, then re-smooths the normals so the surface reads
 *  as a soft rounded bubble, not crumpled low-poly facets.
 *
 *  NOTE: IcosahedronGeometry is NON-indexed in three (co-located duplicates
 *  exist), so the displacement keys off the *quantized vertex position*, not
 *  the array slot: every duplicate gets the SAME offset and the surface stays
 *  watertight. Consumes exactly one rand() (a salt) so call order — and thus
 *  determinism — is independent of the vertex count. */
 ...
 *  hue-neutral, multiplying the leaf map. Assumes rotation preserves Y. */
...
// A leafy shrub: ONE dome — the "stack" lives in the LEAVES: the leaf map
// carries the layered-leaf read, a strong scalloped displacement makes the
// silhouette bulge like leaf clumps, and the layered bake shades it. Wrapped
// in a named 'canopy' group with a gentle windAmp so the wind spring rustles
// it like the trees.
function bush(rand: Rand): THREE.Object3D[] {
  ...
  // Attach the map on LOAD, not eagerly: a material pointing at a texture
  // whose pixels haven't arrived renders BLACK, so the LEAF tint holds the
  // fallback until they do. (No-op in a DOM-less env — vitest never reaches
  // TextureLoader.)
  ...
  // whole mound gently (windAmp well under the trees' — shrubs rustle).
...
/** Stylized model for `kind`, centered on X/Z with its base at y=0 and a
 *  ~1-unit footprint (callers scale/position uniformly). Deterministic given
 *  `seed`. The GLB assets are authored to the same contract. */
export function buildObjectModel(kind: ProceduralKind, seed = 1): THREE.Group {
  ...
  // Ground the model: shift every child up so the lowest point sits at y=0.
  // We move the CHILDREN (not group.position) so a caller can freely set
  // group.position without clobbering the grounding.

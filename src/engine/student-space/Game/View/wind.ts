// Ported from island-editor/src/scene/wind.ts — behavior kept in sync via
...
//     lets go, and rings down naturally.
//
// Pure math, no three.js scene objects — everything is a function of
// (t, position, phase), so the sim is deterministic and testable in node.
...
export class CanopySpring {
  ...
  step(t: number, rawDt: number, worldX: number, worldZ: number, amp: number): void {
    ...
    // the magnitude, a small fast per-tree flutter roughens it, and windAmp
    // scales it per kind (tree 0.55 > bush 0.25).
    const gust = gustStrength(t, worldX, worldZ)
    const flutter =
      0.012 * Math.sin(t * 2.9 + this.phase) + 0.008 * Math.sin(t * 4.3 + this.phase * 2.1)
    ...
    // spring's live state, so the squash rings with the overshoot), over a
    // slow idle breathing so a becalmed tree still feels alive.

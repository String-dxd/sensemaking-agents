// Ported from island-editor/src/scene/materials/GrassBladeMaterial.ts —
// behavior kept in sync via shared test vectors (see State/islandSpecCore/
// terrainGrid.ts provenance note).
//
// BOTW-style procedural grass blade shader. One tapered blade card (5 verts /
// 3 tris, base at y=0, unit height, uv.y 0 at base → 1 at tip) is instanced
// per blade via InstancedBufferGeometry; per-instance attributes:
//
...
//
// r149 PORT NOTES (KTD-4): output conversion via `encodings_fragment`.
// ENGINE ADDITION (KTD-8): a `uDayTint` day-cycle uniform multiplying the
// final blade color, driven from the day-cycle state in the Grass view's
// update() so night/sunset scenes darken the blades with the world.
...
export interface GrassBladeOptions {
  ...
  hideEnd?: number
  /** Max per-blade wind-direction offset in radians (±). */
  dirSpread?: number
...
void main() {
  ...
  // Distance behavior: mid-range blades WIDEN a little so they stay crisp
  // instead of dissolving into sub-pixel noise, then past uHideStart the whole
  // card SHRINKS smoothly to nothing — zoomed-out views deliberately hide the
  // grass (declutter), and it grows back on zoom-in.
  ...
  // (radians) about the blade base. aShadePhase.x doubles as gust
  // susceptibility: only some blades whip hard, the rest just lean.
  ...
  // Bend is a 2-D VECTOR (radians): wind + character push compose, then one
  // sin/cos rotation about the base applies the total.
...
export function createGrassBladeMaterial(opts: GrassBladeOptions = {}): THREE.ShaderMaterial {
  return new THREE.ShaderMaterial({
    ...
    // alpha blending: ~131k instances render in ONE mesh and cannot be

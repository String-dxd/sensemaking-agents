// Ported from island-editor/src/scene/materials/IslandGroundMaterial.ts —
// behavior kept in sync via shared test vectors (see State/islandSpecCore/
// terrainGrid.ts provenance note).
//
// Ground material for the terraced island, keyed off the geometry attributes
// written by islandGeometry.ts (aTierFlat, aWallness, aSurface). BOTW-style
// painterly lighting: warm sun, cool sky ambient, soft toon curve.
//
// r149 PORT NOTES (KTD-4): the editor targets three r171; this port swaps the
// output-conversion chunk to the r149 name (`encodings_fragment` — the r152+
// rename silently no-ops here, enforced by test/engine/colorspace-guard.test.ts)
// and loads the sand/cliff color maps with `texture.encoding = sRGBEncoding`.
//
...
// directionalShadowMap/-Matrix uniforms, etc.). The vertex shader follows
// three's own chunk contract (beginnormal_vertex → defaultnormal_vertex →
// worldpos_vertex → shadowmap_vertex) so `vDirectionalShadowCoord` is written
// correctly; the fragment calls the standard getShadowMask().
...
export interface GroundOptions {
  ...
  /** World-space sun direction (normalized inside). Defaults to the editor
   *  Backdrop directional light's position [18, 20, 10]. */
   ...
   *  texture only begins above it — sand-only shoreline. */
  beachTop?: number
}

/** The editor's authored daylight tints — exported so the view's day-cycle
 *  modulation can scale from these exact noon values (KTD-8). */
export const GROUND_SUN_COLOR = 0xffedcc
export const GROUND_SKY_COLOR = 0x8fa8c8
export const GROUND_GRASS_COLOR = 0x4a8f3f
...
// Fresh value noise (own constants — not the legacy islandHash).
...
void main() {
  ...
  // seaLevel). Band end must stay below the beach freeboard (tier 1 top −
  // seaLevel, 0.05) so the flat top reads dry and the wet gradient stays a
  // rim effect.
  ...
  // toward the grass tone so patch edges blend instead of sitting on bare
  // sand. Land only (tier ≥ 1) — paint on water cells stays invisible.
  ...
  // diagonal cliff, so max() in the geometric slope: a genuinely steep face
  // reads as cliff regardless of orientation.
  ...
  // Sand-only shoreline: the beach tier's little drop to the sea is
  // geometrically steep, so it classified as cliff — a brown lip along every
  // shore. Cliff texture only begins ABOVE the beach top; at and below it,
  // steep faces keep the flat (sand) color, and the wet-sand darkening
  // handles the waterline.
...
export function createIslandGroundMaterial(
  ...
  const uniforms = THREE.UniformsUtils.merge([
    THREE.UniformsLib.lights,
    {
      ...
      uGrassColor: { value: new THREE.Color(opts.grassColor ?? GROUND_GRASS_COLOR) },
      uSunDirection: { value: new THREE.Vector3() },
      uSunColor: { value: new THREE.Color(opts.sunColor ?? GROUND_SUN_COLOR) },
      uSkyColor: { value: new THREE.Color(opts.skyColor ?? GROUND_SKY_COLOR) },
      uSeaLevel: { value: opts.seaLevel ?? 0 },
      uBeachTop: { value: opts.beachTop ?? 0.05 },
    },
  ])
  if (uniforms.uSandTexture) uniforms.uSandTexture.value = textures.sand
  if (uniforms.uCliffTexture) uniforms.uCliffTexture.value = textures.cliff
  if (uniforms.uSunDirection) uniforms.uSunDirection.value = sun

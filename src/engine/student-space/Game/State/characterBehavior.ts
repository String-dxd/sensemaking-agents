// Ported from island-editor/src/models/characterBehavior.ts — behavior kept
// in sync via shared test vectors (see islandSpecCore/terrainGrid.ts; the
// editor's own characterBehavior test suite is ported alongside).
//
...
import type { CharacterClip } from './characterAsset.ts'
...
export function sampleShoreDistance(field: ShoreField, worldSize: number, x: number, z: number): number {
  ...
  return field.data[j * field.res + i] ?? 0
}

// Ported from island-editor/src/editor/gridCodec.ts — behavior kept in sync
// via shared test vectors (see terrainGrid.ts provenance note).
//
// Serialize the grid as arrays of digit strings (one string per row) — human-
// readable, git-diffable, ~4 KB per layer. Digits are the integer codes, capped
// at 9 (MAX_TIER = 4, fine). NO three imports.

import { MAX_TIER, SURFACE_GRASS, type TerrainGrid } from './terrainGrid.ts'
